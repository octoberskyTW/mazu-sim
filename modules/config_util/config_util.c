    ...
    *numEntries = 0;
    ...
    while (variable != NULL) {
        ...
        if (0 == strcmp(type, "STRING")) {
            ...
            fprintf(stderr, "ERROR: unrecognized specifier type %s! Please check your\n", type);
            fprintf(stderr," input or check read_config.cpp. Available types: STRING/ENUM/NUMBER\n");
            return CONFIG_ERROR; 
        }
    ...
    while (1) {
        ...
        if (numFieldsRead == numFields) {
            ...
            // The current pointer so we can access the struct dynamically by memory
            char* currPointer = (char*)table + structSize * i;
            for (numFieldsRead = 0; numFieldsRead < numFields; numFieldsRead++) {
                field = fieldArray[numFieldsRead];
                // Do not set values if too many fields, to avoid overrunning buffer
                if (fieldSizes[numFieldsRead] > 0) {
                    // Fill out the appropriate field in the struct
                    switch (fieldTypes[numFieldsRead]) {
                    case CONFIG_STRING:
                        // Copy 1 smaller than the size for the null-terminated string
                        strncpy(currPointer, field, fieldSizes[numFieldsRead] - 1);
                        break; 
                    case CONFIG_ENUM:
                        // Translate name of enum to enum value, which is an integer 
                        enumVal = get_enum_value(field); 
                        if (enumVal == -1) { 
                            fprintf(stderr, "ERROR: Aborting. Do not pass Go.\n");
                            return CONFIG_ERROR;
                        }
                    ...
                    case CONFIG_NUMBER:
                        ...
                        *(int*)currPointer = parsedVal;
                        break;
                    default:
                        // This shouldn't happen, would be something wrong with memory maybe?
                        fprintf(stderr, "ERROR: invalid type for the field (check read_config.cpp code)\n");
                        fprintf(stderr, "ERROR: Aborting. Do not pass Go. Do not collect $200.\n");
                        return CONFIG_ERROR;
                        break;
                    } // switch (fieldTypes[numFieldsRead]) 
                } // if (numFieldsRead < numFields)
            ...
            // Didn't get correct number of fields, but this may be intended (like for atomintr)
            fprintf(stderr, "WARNING: Expected %d fields; got %d while reading line %d\n", numFields, numFieldsRead, linenum);
            error++;
        }
